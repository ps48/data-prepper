...
public class OTelTelemetrySource implements Source<Record<Object>> {
    ...
    public void start(Buffer<Record<Object>> buffer) {
        ...
        grpcServiceBuilder.addService("/v1/logs", ServerInterceptors.intercept(new OTelLogsGrpcService(buffer, logsDecoder)));
        grpcServiceBuilder.addService("/v1/metrics", ServerInterceptors.intercept(new OTelMetricsGrpcService(buffer, metricsDecoder)));
        grpcServiceBuilder.addService("/v1/traces", ServerInterceptors.intercept(new OTelTracesGrpcService(buffer, tracesDecoder)));
    ...
    @Override
    public void process(Record<Object> record) {
        if (record.getData() instanceof TelemetryData) {
            TelemetryData telemetryData = (TelemetryData) record.getData();
            switch (telemetryData.getType()) {
                case LOG:
                    processLog(telemetryData.getData());
                    break;
                case METRIC:
                    processMetric(telemetryData.getData());
                    break;
                case TRACE:
                    processTrace(telemetryData.getData());
                    break;
            }
        }
    }

    private void processLog(Object logData) {
        // Implement logic to process log data
        LOG.info("Processing log data: {}", logData);
    }

    private void processMetric(Object metricData) {
        // Implement logic to process metric data
        LOG.info("Processing metric data: {}", metricData);
    }

    private void processTrace(Object traceData) {
        // Implement logic to process trace data
        LOG.info("Processing trace data: {}", traceData);
    }
}